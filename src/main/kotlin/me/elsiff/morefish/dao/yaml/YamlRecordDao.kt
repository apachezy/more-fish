...
class YamlRecordDao(
    ...
    override fun update(record: Record) {
        ...
        require(section != null) { "Record must exist in the ranking" }
        setRecord(section, record)
        yaml.save(file)
    }
    ...
    override fun all(): List<Record> {
        ...
        for (section in yaml.getKeys(false).map(yaml::getConfigurationSection)) {
            ...
            val fish = fishLength?.let { Fish(fishType, it) }
            fish?.let { Record(player, it) }?.let { records.add(it) }
        }
